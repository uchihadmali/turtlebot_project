...
int main(int argc, char** argv)
{
        ...
        std::cout << "Leader module started!" << std::endl;
        obs_pos_pub = obsPosPub.advertise<geometry_msgs::Pose>("/obs_pos", 1000);//publisher for position at gazebo
        while (ros::ok()){
        ...
        obs_pos_pub.publish(pose_msg);
	targetPositionX=-1;
	targetPositionY=-1; //publishes position message
        ros::spinOnce();
        loop_rate.sleep();
        }
