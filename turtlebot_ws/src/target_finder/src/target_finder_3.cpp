...
double targetPositionX=-1;
double targetPositionY=-1;
class TargetFinder
{
    ros::NodeHandle nh_;
    ...
    : it_(nh_)
  {
      ...
      detection_pub = it_.advertise("/detection_result_leader", 1);
  }
  ...
  void depthCb(const sensor_msgs::ImageConstPtr& msg)
  {
      ...
      if(target_found)
      {
          ...
          {
              targetPositionY = target_distance * sin((kinectFovX* ((posX - 320)/ 320.0))* (PI/180));
              targetPositionX = sqrt(pow(target_distance,2) - pow(targetPositionY,2));
...
{
        ros::init(argc, argv, "target_finder3");
        ros::NodeHandle parameterServer("~");
        ros::NodeHandle obsPosPub;
        ros::Rate loop_rate(LOOP_RATE);
        ros::Publisher obs_pos_pub;
        

        //get params from server
        parameterServer.getParam("/low_H_follower",low_H);
        parameterServer.getParam("/high_H_follower",high_H);
        parameterServer.getParam("/low_S_follower",low_S);
        parameterServer.getParam("/high_S_follower",high_S);
        parameterServer.getParam("/low_V_follower", low_V);
        parameterServer.getParam("/high_V_follower", high_V);
        ...
        std::cout << "Leader finder module started!" << std::endl;
        obs_pos_pub = obsPosPub.advertise<geometry_msgs::Pose>("/leader_pos", 1000);//publisher for position at gazebo
        while (ros::ok()){
        geometry_msgs::Pose pose_msg; //position message
        pose_msg.position.x=targetPositionX;
        pose_msg.position.y=targetPositionY;
        ...
        obs_pos_pub.publish(pose_msg); //publishes position message
        ros::spinOnce();
        loop_rate.sleep();
        }
